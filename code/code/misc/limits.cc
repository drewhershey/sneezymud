...
int graf(const TBeing* tb, int age, int p0, int p1, int p2, int p3, int p4,
  int p5, int p6) {
...
int eqHpBonus(const TPerson* ch) {
  ...
  for (i = MIN_WEAR; i < MAX_WEAR; i++) {
    for (j = 0; j < MAX_OBJ_AFFECT; ++j) {
      if (ch->equipment[i] && (o = dynamic_cast<TObj*>(ch->equipment[i])) &&
...
float classHpPerLevel(const TPerson* tp) {
  ...
  if (!hpgain || num_classes == 0) {
    vlogf(LOG_BUG,
      format("No class in classHpPerLevel() for %s") % tp->getName());
    hpgain = 7.0;
  }
...
int ageHpMod(const TPerson* tp) {
  ...
  if (!tp)
    return 0;
  return graf(tp, (tp->age()->year - tp->getBaseAge() + 15), 2, 4, 17, 14, 8, 4,
    3);
}
...
int TMonster::hitGain() {
  ...
  // i'm also gonna cap level at 50 so that there is no way we can get negative
  // values

  int oldgain = max(2, (GetMaxLevel() / 2));
  // the old version

  // base1 is the % for level 1, base50 is the percent for level 50, for easy
  // adjusting
  ...
  gain =
    (int)((base1 - (base1 - base50) * (level / 50.0)) * (double)hitLimit());

  // just because i'm an asshole, i'm going to make sure this new value wont be
  // lower than the old value
...
int TPerson::hitGain() {
  ...
  if (fight())
    ...
    gain += 4;
    gain =
      (int)((double)(gain)*plotStat(STAT_CURRENT, STAT_CON, .80, 1.25, 1.00));
  }
...
double TBeing::pracsPerLevel(classIndT Class, bool forceBasic) {
  ...
  double advancedpracs =
    (getAdvancedPracs(this, Class) * learnrate) / (50.0 - advancedlevel);
...
short TBeing::calcNewPracs(classIndT Class, bool forceBasic) {
  ...
  /*  if(isPc()) {
      vlogf(LOG_DASH, format("%s gaining %d pracs roll = %d (%d + %4.2f) lev:
     %d") %  getName() % prac % roll % (int)temp % num % getLevel(Class));
      }*/
...
void TPerson::fixPracs() {
  ...
  // if the person is under by more than level/10 and this is their first time,
  // give them a second chance
  if (-diff > max(5, level / 2) && hasQuestBit(TOG_PRACS_FIXED)) {
    vlogf(LOG_BUG, format("%s was eligible to have practices corrected, but "
                          "has already had that done and was skipped.") %
                     getName());
  } else if (diff > level / 10 && !hasQuestBit(TOG_PRACS_FIXED)) {
    vlogf(LOG_MISC, format("%s is having practices corrected, got an extra %d "
                           "pracs.  Had %d unspent.") %
                      getName() % diff % getPracs(Class));
    addPracs(diff, Class);
    sendTo(format("%sYou have gained %d practices as a result of an account "
                  "update.%s\n\r") %
           redBold() % diff % norm());
    ...
    if (getMaxExp() > 1) {
      ...
      vlogf(LOG_BUG, format("%s has %d pracs too many and has had the no "
                            "experience toggle set.  Prepare for unrest.") %
                       getName() % -diff);
      sendTo(format("%sSomething has gone drastically wrong with your practice "
                    "allocation.  Experience gain has been disabled for this "
                    "player.  Contact a creator.%s\n\r") %
             redBold() % norm());
      setQuestBit(TOG_NO_XP_GAIN);
      doSave(SILENT_YES);
    } else {
      vlogf(LOG_BUG,
        format("%s may or may not have excess pracs.  Max XP was zero.  %s has "
               "been instructed to kill something then rent and come back.") %
          getName() % getName());
      sendTo(
        format(
          "%sThis character is old and needs to be updated before practices "
          "can be evaluated.  You can cause this to happen automatically.  "
          "Please go kill something (anything, e.g. a fuzzy mouse), then rent, "
          "and come back.  Thanks for your help.%s\n\r") %
        redBold() % norm());
    }
  }
}
...
short TBeing::expectedPracs() {
  ...
  for (Class = MIN_CLASS_IND; Class < MAX_CLASSES; Class++) {
    ...
    /*
       ...
       advancedlevel is %f") % level % getMaxExp() % fraction % advancedlevel );
       */
    for (i = 0; i <= level; i++) {
...
void TPerson::setTitle(bool tForm) {
  char tString[256] = "\0", tBuffer[256] = "\0";
...
void gain_exp(TBeing* ch, double gain, int dam) {
  ...
  if (ch->hasQuestBit(TOG_NO_XP_GAIN)) {
    ch->sendTo(format("%sYou would have gained XP here, but there is a problem "
                      "with your account.  Please contact a creator.%s\n\r") %
               ch->redBold() % ch->norm());
    return;
  }

  gain /= ch->howManyClasses();  // exp gets later on applied separately for
                                 // each class. Divide to avoid leveling faster
                                 // than regular characters
  gain /= ch->howManyClasses();  // to balance multiclass players, have them
                                 // gain exp more slowly
  ...
  for (Class = MIN_CLASS_IND; Class < MAX_CLASSES; Class++) {
    ...
    if (ch->isPc() && ch->getMaxExp() == 0) {
      vlogf(LOG_DASH, format("%s getting exp checked: MaxExp %.2f, Exp %.2f, "
                             "Current Level Exp %.2f") %
                        ch->getMaxExp() % ch->getExp() % getExpClassLevel(50) %
                        curr);
    ...
    if ((ch->getExp() + gain) > ch->getMaxExp()) {
      ...
      if (ch->getLevel(Class) < MAX_MORT) {
        ...
        if (ch->getMaxExp() > t_peak) {
          vlogf(LOG_MAROR,
            format("%s has more experience than normal for level, gaining "
                   "pracs for a whole level to make up for it.") %
              ch->getName());
          exp = t_curr;
        }
        ...
        for (t_exp = t_curr + delta_exp; t_exp <= new_exp && t_exp <= t_peak;
             t_exp += delta_exp) {
          // if this exp step is past our max exp and is under the exp
          // we've gained, then get a prac
          if (t_exp > exp && t_exp <= new_exp) {
            vlogf(LOG_SILENT,
              format("%s gaining practice (current): t_curr=%f, t_peak=%f, "
                     "delta_exp=%f, exp=%f, new_exp=%f, t_exp=%f") %
                ch->getName() % t_curr % t_peak % delta_exp % exp % new_exp %
                t_exp);
            ;
            gain_pracs++;
          }
        ...
        if (new_exp >=
            t_peak) {  // t_peak is the amount of exp required to level
          // roll for extra prac
          double overshoot =
            t_exp - t_peak;  // amount of delta that is not part of this level
          if (::number(1, (int)delta_exp) < (delta_exp - overshoot)) {
            vlogf(LOG_SILENT,
              format("%s gaining practice (threshold): t_curr=%f, t_peak=%f, "
                     "delta_exp=%f, exp=%f, new_exp=%f, overshoot=%f") %
                ch->getName() % t_curr % t_peak % delta_exp % exp % new_exp %
                overshoot);
            gain_pracs++;
          }
      ...
      // if(new_exp > peak)
        ...
        // max to min
        //  if (ch->getLevel(Class) >= MAX_MORT)
        exp = ch->getMaxExp();  // setting min exp required to get a prac in
                                // this level, for the loop
        ...
        for (j = t_curr; j <= new_exp && j <= t_peak; j += delta_exp) {
          if (j > exp && j < new_exp) {
            vlogf(LOG_SILENT,
              format("%s gaining practice (next): t_curr=%f, t_peak=%f, "
                     "delta_exp=%f, exp=%f, new_exp=%f, j=%f") %
                ch->getName() % t_curr % t_peak % delta_exp % exp % new_exp %
                j);
            ;
            gain_pracs++;
        ...
        if (new_exp >=
          ...
          // roll for extra prac
          double overshoot =
            new_exp - t_peak;  // amount of delta that is not part of this level
          if (::number(1, (int)delta_exp) < (delta_exp - overshoot)) {
            vlogf(LOG_SILENT,
              format("%s gaining practice (threshold): t_curr=%f, t_peak=%f, "
                     "delta_exp=%f, exp=%f, new_exp=%f, overshoot=%f") %
      ...
      //// ADD PRACTICES
      // THE CONDITIONAL ON THE FIRST LINE IS TO FIX SOME LONG TERM PROBLEMS
      // WITH EXCESS PRACTICES BEING GIVEN OUT
      if (gain_pracs > 0) {
        if (ch->hasQuestBit(TOG_PRACS_FIXED) &&
            ch->meanPracsSoFar() >
              ch->expectedPracs() + ch->getLevel(Class) / 10)
          vlogf(LOG_MAROR, format("%s gaining practice outside normal bounds "
                                  "with 'practices ok' toggle on.") %
                             ch->getName());
        if (ch->hasQuestBit(TOG_PRACS_FIXED) ||
            ch->meanPracsSoFar() <=
              (ch->expectedPracs() + ch->getLevel(Class) / 10)) {
          ...
          if (gain_pracs == 1) {
            ...
            ch->sendTo(COLOR_BASIC,
              format("<W>You have gained %i practices!<1>\n\r") % gain_pracs);
          }

        } else
          vlogf(LOG_MAROR,
            format("%s missing practice gain due to too many practices.") %
              ch->getName());
      }
    }
    ...
    // (the function is sometimes called with zero damage, should not gain
    // experience in these cases, and therefore no level) check for level gain
...
void TFood::findSomeFood(TFood** last_good, TBaseContainer** last_cont,
  TBaseContainer* cont) {
    ...
    switch (condition) {
      ...
      case DRUNK:
        ...
        value =
          (int)((double)value *
                (double)((
                  (double)(105 - getSkillValue(SKILL_ALCOHOLISM)) / 100)));
        break;
      case PEE:
      case POOP:
      case MAX_COND_TYPE:
    ...
    switch (condition) {
      ...
      case DRUNK:
        if (inRoom() >= 31800 && inRoom() <= 31899) {
          sendTo(
            "As you begin to sober up, your grasp on this strange world begins "
            "to loosen.\n\r");
        }
...
int TBeing::checkIdling() {
  ...
  if (getTimer() == 10) {
    ...
    if (in_room != Room::STORAGE) {
      ...
      vlogf(LOG_SILENT,
        format("%s booted from game for inactivity.") % getName());
