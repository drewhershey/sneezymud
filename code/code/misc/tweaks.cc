...
tweakEntry* tweakInfoT::operator[](const tweakTypeT t) { return tweaks.at(t); }

tweakTypeT& operator++(tweakTypeT& c, int) {
  return c = (c == MAX_TWEAK_TYPES) ? TWEAK_LOADRATE : tweakTypeT(c + 1);
}

tweakInfoT::tweakInfoT() : tweaks(MAX_TWEAK_TYPES + 1, nullptr) {}
...
int tweakInfoT::loadTweaks() {
  ...
  // On load fill out all tweaks with 1.0.  This will make adding new tweaks
  // easier by eliminating the need to write a migration.
  ...
  // we do a greatest n per group query
  db.query(
    "select tweak_id, tweak_type, tweak_value, tweak_target, tweak_rate from "
    "globaltweaks a "
    "where tweak_id = (select max( tweak_id ) from globaltweaks b where "
    "a.tweak_type = b.tweak_type)");
  // enter in the proper values. Is it possible for the query to fail and the
  // mud still be running?
  int i = 0;
  while (db.fetchRow()) {
...
void tweakInfoT::handleTweak(TBeing* b, tweakTypeT t, sstring arg) {
  ...
  if (arg.empty()) {
    if (is_abbrev(opt, "up") || is_abbrev(opt, "down")) {
      ...
      sneezy.query(
        "insert into globaltweaks (tweak_type, tweak_value, tweak_target, "
        "tweak_rate) values (%i,%f,%f,%f)",
      ...
      b->sendTo(COLOR_BASIC,
        ...
        "----------<1>\n\r");
      b->sendTo(COLOR_BASIC,
        format("<c>%-15f | %-12f | %-12f | %-24s <1>\n\r") % "Value" %
          "Target" % "Change Rate" % "Date");
      sneezy.query(
        "select tweak_id, tweak_type, tweak_value, tweak_target, tweak_rate, "
        "datecreated from globaltweaks where tweak_type = %i",
        t);
  ...
  // multiple arguements given after type
    ...
    sstring usage =
      "Usage: info tweak <parameter> set <target> [transition time in "
      "seconds]\r\n";
    ...
    if (!is_number(opt) || convertTo<double>(opt) < 0) {
      b->sendTo(
        "Invalid value given for global potential. Must be a number >= "
        "0.0.\r\n " +
        usage);
      return;
    }
    ...
    if (arg.empty()) {
      ...
      vlogf(LOG_MISC, format("%s set %s to %f") % b->getName() %
                        getTweakName(t) % tweak->current);
      b->sendTo(format("Target global %s set to %f\r\n") % getTweakName(t) %
                tweak->target);
      sneezy.query(
        "insert into globaltweaks (tweak_type, tweak_value, tweak_target, "
        "tweak_rate) values (%i,%f,%f,0.0)",
    ...
    if (!is_number(opt) || convertTo<double>(opt) <= 0) {
      b->sendTo(
        "Invalid value given for transition time. Must be a positive number of "
        "seconds.\r\n " +
        usage);
      return;
    }
    ...
    tweak->rate =
      fabs((tweak->target - tweak->current) / change_period_seconds);
    b->sendTo(
      format("Target global %s set at %f to change over %f seconds.\r\n") %
      getTweakName(t) % tweak->target % convertTo<int>(opt));
    vlogf(LOG_MISC,
      format(
        "%s set the target global %s at %f to change over %f seconds.\r\n") %
    ...
    // proc tweak rates does not create any new rows in db, only updates
    // current.  To make history understandable, enter two rows - the first
    // indicates the change date, the second holds the current tweak struct
    sneezy.query(
      "insert into globaltweaks (tweak_type, tweak_value, tweak_target, "
      "tweak_rate) values (%i,%f,%f,%f)",
      t, tweak->current, tweak->target, tweak->rate);
...
void tweakInfoT::showTweaks(TBeing* b, tweakTypeT t) {
  ...
  b->sendTo(COLOR_BASIC, format("<c>%-22s : %-15f | %-15f | %-15f <1>\n\r") %
                           "Tweak" % "Current Value" % "Target" %
                           "Change Rate");
  if (t == MAX_TWEAK_TYPES) {
    for (tweakTypeT i = TWEAK_LOADRATE; i < MAX_TWEAK_TYPES; i++) {
      b->sendTo(COLOR_BASIC, format("%-22s : %-15f | %-15f | %-15f \n\r") %
                               tweakInfo.getTweakName(i) %
                               tweakInfo[i]->current % tweakInfo[i]->target %
                               tweakInfo[i]->rate);
    }
    b->sendTo(COLOR_BASIC,
      "<c>---------------------------------------------------------------------"
    ...
    b->sendTo(COLOR_BASIC, format("%-22s : %-15f | %-15f | %-15f \n\r") %
                             tweakInfo.getTweakName(t) % tweakInfo[t]->current %
                             tweakInfo[t]->target % tweakInfo[t]->rate);
    b->sendTo(COLOR_BASIC,
      "<c>---------------------------------------------------------------------"
      "--------<1>\n\r");
  }
...
void tweakInfoT::showTweakOptions(TBeing* b) {
  ...
  str +=
    "\tinfo tweak <parameter> set <target> <transition time in seconds>\r\n";
  b->sendTo(str);
}
...
const sstring tweakInfoT::getTweakName(tweakTypeT tt) {
  switch (tt) {
    ...
    case TWEAK_SKILLTRAININGCOST:
      return "SkillTrainingCost";
