...
class TMonster : public TBeing {
  public:
    ...
    int greed() const { return opinion.greed; }
    int anger() const { return opinion.anger; }
    int malice() const { return opinion.malice; }
    int susp() const { return opinion.suspicion; }
    void setGreed(int res) { opinion.greed = res; }
    void setAnger(int res) { opinion.anger = res; }
    void setMalice(int res) { opinion.malice = res; }
    void setSusp(int res) { opinion.suspicion = res; }
    int defgreed() const { return opinion.def_greed; }
    int defanger() const { return opinion.def_anger; }
    int defmalice() const { return opinion.def_malice; }
    int defsusp() const { return opinion.def_suspicion; }
    void setDefGreed(int res) { opinion.def_greed = res; }
    void setDefAnger(int res) { opinion.def_anger = res; }
    void setDefMalice(int res) { opinion.def_malice = res; }
    void setDefSusp(int res) { opinion.def_suspicion = res; }
    TBeing* targ() const { return opinion.target; }
    void setTarg(TBeing* ch) { opinion.target = ch; }
    bool isGreedy() const { return (::number(0, 101) < greed()); }
    bool isAngry() const { return (::number(0, 101) < anger()); }
    ...
    void DG(int num) { setGreed(greed() - min(::number(0, 2 * num), greed())); }
    void UA(int num) {
      setAnger(anger() + min(::number(0, 2 * num), 100 - anger()));
    }
    void DA(int num) { setAnger(anger() - min(::number(0, 2 * num), anger())); }
    void UM(int num) {
