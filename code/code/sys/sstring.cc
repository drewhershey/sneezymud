...
void br() { vlogf(LOG_BUG, "sstring::escape(): TODO implement"); }
...
const sstring sstring::ansiToAard() const {
  ...
  // colors with styles
  oBuf.inlineReplaceString(ANSI_RED_BOLD, "@R");
  oBuf.inlineReplaceString(ANSI_GREEN_BOLD, "@G");
  oBuf.inlineReplaceString(ANSI_ORANGE_BOLD, "@Y");
  oBuf.inlineReplaceString(ANSI_YELLOW_BOLD, "@Y");
  oBuf.inlineReplaceString(ANSI_BLUE_BOLD, "@B");
  oBuf.inlineReplaceString(ANSI_PURPLE_BOLD, "@M");
  oBuf.inlineReplaceString(ANSI_CYAN_BOLD, "@C");
  oBuf.inlineReplaceString(ANSI_WHITE_BOLD, "@W");

  // colors with background
  oBuf.inlineReplaceString(ANSI_BK_ON_BK, "@x000");
  oBuf.inlineReplaceString(ANSI_BK_ON_WH, "@w");
  oBuf.inlineReplaceString(ANSI_WH_ON_BL, "@w");
  oBuf.inlineReplaceString(ANSI_WH_ON_CY, "@c");
  oBuf.inlineReplaceString(ANSI_WH_ON_GR, "@g");
  oBuf.inlineReplaceString(ANSI_WH_ON_OR, "@y");
...
sstring sstring::join(const std::vector<sstring>& words,
  const sstring& delimiter) {
...
const sstring sstring::trim() const { return trimLeft().trimRight(); }
...
const sstring sstring::matchCase(const sstring match) const {
  ...
  while (iMatch < (int)match.length() && iOut < (int)out.length()) {
    ...
    if (match[iMatch] == ' ') {
      while (iMatch < (int)match.length() && match[iMatch] == ' ')
        iMatch++;
      if (out[iOut] != ' ')
        while (iOut < (int)out.length() && out[iOut] != ' ')
          iOut++;
      while (iOut < (int)out.length() && out[iOut] == ' ')
        iOut++;
      continue;
...
size_t sstring::findBetween(const sstring start, const sstring toFind,
  const sstring end) const {
  sstring data = upper();
  size_t iStart = 0;
  while (1) {
