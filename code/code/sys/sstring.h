...
class sstring : public std::string {
  public:
    ...
    static sstring join(const std::vector<sstring>& words,
    ...
    // takes one set of markup tags if they exist in order, and replaces then
    // with new tags in the same order
    void inlineReplaceMarkup(const sstring markupStart, const sstring markupEnd,
      const sstring replaceStart, const sstring replaceEnd) {
