...
describe("Mail System", () => {
  ...
  it("sends and receives mail round-trip", async () => {
    ...
    // where normal prompt detection doesn't apply.
    await mud.send(`mail ${config.character}`);
    // Wait for the postmaster to acknowledge and enter writing mode
    await mud.readUntilIdle(1);

    // Write the message body and terminate with ~
    await mud.send(marker);
    const composeOutput = await mud.rawSend({ text: "~" });
    expect(composeOutput).toContainCaseInsensitive("Message sent");

    // Drain any trailing output before the next command
